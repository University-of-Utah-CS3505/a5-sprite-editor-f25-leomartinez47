...
QColor colorFromJson(QJsonArray rgb) {
    return QColor(rgb.takeAt(0).toInteger(), rgb.takeAt(0).toInteger(),
                  rgb.takeAt(0).toInteger());
}

Project::Project(QSize dimensions, QObject *parent) : QObject{parent} {
...
Project::Project(const QString &path, QObject *parent) : QObject{parent} {
    ...
    if (!json.contains("sprite") || !json.contains("currentFrame") ||
        !json.contains("currentTool") || !json.contains("currentColor")) {
        throw std::invalid_argument(
            "Project information could not be retrieved.");
    }
...
void Project::onPixelClicked(QPoint point) {
    this->currentTool->apply(point, this->getCurrentFrame(),
                             this->currentColor);
}
...
QJsonObject Project::toJson() {
    return QJsonObject({{"sprite", this->sprite->toJson()},
