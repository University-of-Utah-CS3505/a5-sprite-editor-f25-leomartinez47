...
PreviewPane::PreviewPane(Project *project, QWidget *parent)
    : QWidget(parent), ui(new Ui::PreviewPane), currentFrameIndex(0) {
    this->ui->setupUi(this);

    this->sprite = project->getSprite();

    this->ui->frameRateSelector->setRange(0, 60);
    this->ui->frameRateSelector->setValue(30);
    ...
    connect(&timer, &QTimer::timeout, this, &PreviewPane::showFrame);

    connect(this, &PreviewPane::setFrameRate, project,
            &Project::onFrameRateSet);

    connect(ui->frameRateSelector, &QSpinBox::valueChanged, this,
            [this](int fps) {
                this->timer.setInterval(1000 / fps);
                emit setFrameRate(fps);
            });

    connect(project, &Project::stopAnimation, this,
...
void PreviewPane::showFrame() {
    this->update();
    this->currentFrameIndex >= this->sprite->frameCount() - 1
        ? this->currentFrameIndex = 0
...
void PreviewPane::paintEvent(QPaintEvent *) {
    ...
    // Transparency grid
    QRect gridArea = QRect(xOffset, yOffset, scaledWidth, scaledHeight);
    ...
    for (int y = gridArea.top(); y < gridArea.bottom(); y += gridSize) {
        for (int x = gridArea.left(); x < gridArea.right(); x += gridSize) {
            QColor squareColor =
                (((x - xOffset) / gridSize + (y - yOffset) / gridSize) % 2 == 0)
                    ? lightGray
                    : darkGray;
